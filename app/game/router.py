...
async def start_draft_endpoint(payload: StartDraftRequest, db: AsyncSession = Depends(get_db)):
    session_id = await draft_service.start_draft(payload.user_id, payload.formation, db)
    return StartDraftResponse(draft_session_id=session_id)


@router.get("/{session_id}/roll")
async def roll_endpoint(session_id: int, db: AsyncSession = Depends(get_db)):
    result = await draft_service.roll_draft(session_id, db)
    return {
...
async def pass_endpoint(session_id: int, db: AsyncSession = Depends(get_db)):
    result = await draft_service.pass_roll(session_id, db)
...
async def pick_endpoint(session_id: int, payload: PickRequest, db: AsyncSession = Depends(get_db)):
    return await draft_service.pick_player(session_id, payload.player_id, payload.slot_index, db)
